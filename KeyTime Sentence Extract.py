...
start = time.time()
pwd = "halo yells at the party show"
pwdLen = len(pwd)
...
#-----Data Record Options--------------
write = False
fileName = "sentData4.txt" # File to write to
...
#     print("press", pwd[pressIndex], " curr:", key.char)
    if pwd[pressIndex] == " " and key == Key.space:
        timeStack.append(time.time())
        keyStack.append(" ")
        pressIndex += 1
        stackSize += 1
    elif hasattr(key, "char") and key.char == pwd[pressIndex]:
        timeStack.append(time.time())
...
def writetoFile(fname, times):
    ...
    l1 = "I:"
    l2 = outputString
    for i in [0, 11, 21, 5, 6, 7, 2, 26, 27, 28, 39, 49, 33, 34, 35, 30, 54]:
            l1 += f" {times[i]}"
    l1 += "\n"
    f.write(l1)
    f.write(l2 + "\n\n")
